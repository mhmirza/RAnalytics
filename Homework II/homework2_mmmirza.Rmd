...
```{r}
# Import data file
income <- read_csv("http://www.andrew.cmu.edu/user/achoulde/94842/data/income_data.txt", col_names = FALSE)
...
```{r}
sex_status <- table(income$marital.status, income$sex)
...
```{r}
prop.sex_status <- round(prop.table(sex_status, 2), 3)

print(prop.sex_status)
```
...
> A summation of the proportion of females who are married with spouses across different categories (AF, CIV, Absent) is `r sum(prop.sex_status[2:4, 1])*100`% compared to `r sum(prop.sex_status[2:4, 2])*100`% for males. Hence, we can say that women are less likely to be married compared to men. 
The proportion of females who report being widowed is `r prop.sex_status [7, 1]*100`% whereas the proportion of males who report being widowed is `r prop.sex_status [7, 2]*100`%. Hence, we can say that women are more likely to be widowed than men.
...
bank.data %>%
  ...
  summarise(customers = n(), 
            subscribers = sum(y == "yes"),
            percent_subscribed = subscribers/customers)
```
...
The months of March and and December were the most successful months with ~52.0% and ~46.7% of the clients who were contacted, subscribing to the term deposit respectively. September and October followed next with ~46.5% and ~43.8% respective subscription rate among those who were called.
...
More successful months were those where marketers tended to make fewer calls. For example, in March which has the highest proportion of clients who subscribed when contacted i.e. ~52% in fact has the second lowest number of calls across all months i.e. 477. Similarly, December which has the second highest proportion of clients who subscribed when contacted i.e. ~48% in fact has the lowest number of calls across all months i.e. 214. This means the percentage subscription only gives us a partial performance analysis.
...
```{r}
survey.untidy <- read.csv("http://www.andrew.cmu.edu/user/achoulde/94842/data/survey_untidy2020.csv", header=TRUE)

survey.tidy <- mutate(survey.untidy, 
...
survey.tidy <- mutate(survey.tidy,
                      TVhours = as.numeric(gsub("[^0-9.]", "", TVhours)))

print(survey.tidy$TVhours)
```
...
```{r}
table(survey.tidy$Program)
```

**Description of the problem:**
<font color="#157515"><b>

This column has redundant unique values which might distort our analysis. For example, "mism", "Mism" and "MISM" are essentially the same, however due to difference in capitalization, they have been accounted for separately in this data. Similarly, "ppm", "Ppm" and "PPM" should be combined, along with "other" and "Other". 
...
Your function should use the `length()` function to check if `x` has at least `s + l + 1` values.  If `x` is shorter than `s + l + 1`, your function should use the `message()` function to tell the user that the vector can't be trimmed as requested.  If `x` is at least length `s + l + 1`, x your function should return the trimmed mean.

```{r}
# Here's a function skeleton to get you started

# The function calculates the trimmed mean of a numeric vector 'x', ignoring 's' number of smallest values and 'l' number of largest values if the length of x is equal to or greater than 's + l + 1', otherwise prints an error message.

trimmedMean <- function(x, s = 0, l = 0)
  if(length (x) < s + l + 1) {
    message ("Vector cannot be trimmed as requested")
...
for (i in list.random) {
  Tmeans <- append(Tmeans, round (trimmedMean(i, 5, 5), 3))
  }
...
Nmeans <- c()
for (i in list.random) {
  Nmeans <- append(Nmeans, round (mean(i), 3))
  }
...
The trimmed mean for vector 'x' is -0.043 compared to the un-trimmed mean of -0.081. The trimmed mean for vector 'y' is 0.882 compared to the un-trimmed mean of 0.926. The trimmed mean for vector 'z' is 0.081 compared to the un-trimmed mean of 0.110. The un-trimmed mean for x is lower than the trimmed mean, but it is higher for y and z. The differences are due to the variability of the data, which is different for x i.e. normal, y i.e. exponential and z i.e. t-distribution, as evident in the qplots above. Since x is left skewed, y is right skewed and z has outliers, trimming the 5 largest and smallest values from each brings the mean closer to the center of the data.
...
map (list.random, .f = trimmedMean, s = 5, l = 5)

map_dbl(list.random, .f = trimmedMean, s = 5, l = 5)
```
